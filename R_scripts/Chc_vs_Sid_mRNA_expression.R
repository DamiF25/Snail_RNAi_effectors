...
shapiro_results <- data %>%
  group_by(Tissue) %>%
  summarise(
    shapiro_Chc1_p = shapiro.test(na.omit(Chc1))$p.value,
    shapiro_Sid1_p = shapiro.test(na.omit(Sid1L))$p.value
  )
...
y_axis_upper <- max(data_long$Expression, na.rm = TRUE)
...
# Tissues with significant comparisons (exclude non-significant Tissue T)
sig_tissues <- c("AG", "HF", "HP/DG", "OV")
...
p <- ggplot(data_long, aes(x = Tissue, y = Expression, fill = Gene, color = Gene)) +
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", position = position_dodge(width = 0.4), 
      ...
      geom_jitter(aes(color = Gene), position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.4), 
              size = 3.0, alpha = 0.7) +
  stat_compare_means(data = data_long %>% filter(Tissue %in% sig_tissues),
    aes(x = Tissue, y = Expression, group = Gene), 
                     method = "wilcox.test", label = "p.signif",
                     symnum.args = list(
                       cutpoints = c(0, 0.001, 0.01, 0.05, 1),
                       symbols = c("***", "**", "*", "ns")),
                     size = 10, inherit.aes = FALSE,
                     label.y = y_axis_upper + 0.05) + # Asterisk position
  geom_segment(data = data_long %>% filter(Tissue %in% sig_tissues),
  ...
  geom_segment(data = data_long %>% filter(Tissue %in% sig_tissues),
    aes(
      ...
      xend = as.numeric(Tissue) + 0.2,
      y = y_axis_upper * 1.17,
      yend = y_axis_upper * 1.05    # tip drops down by 0.07
    ),
    inherit.aes = FALSE,
    color = "black",
    linewidth = 0.5
...
#NOTE: Ignore the `position_dodge()` warning message or reduce bar width
