...
# Load the data
data <- read.csv("Dcr1_mRNA_expression.csv")

# Robust MAD-based outlier detection function
...
# Apply outlier detection within each Size group
outlier_results <- data %>%
  group_by(Size) %>%
  mutate(is_outlier = flag_outliers(Delta_Ct))

# Show which replicates were flagged
print(outlier_results %>% filter(is_outlier == TRUE))

# Remove outliers from dataset
clean_data <- outlier_results %>% filter(is_outlier == FALSE)

# Use clean_data for all downstream analyses
data <- clean_data
...
p <- ggplot(data, aes(x = Size, y = Delta_Ct, fill = Size)) +
  ...
  labs(title = "Dcr1", x = "Size group", y = "Relative expression") +
  theme_test() +
  ...
  theme(plot.title = element_text(size = 24, face = "bold", hjust = 0.5),  # Title size & bold
        ...
        axis.line = element_line(linewidth = NULL),    
...
# Save figure
ggsave(filename = "Dcr1 mRNA expression levels.tif",
       plot = p,
       width = 5,
       height = 5,
       dpi = 1000,
