...
# Apply outlier detection within each Tissue group
...
# Y-axis expansion
y_axis_upper <- max(data$Delta_Ct) * 1.2
...
if (nrow(signif_comparisons) > 0) {
  p <- p + geom_signif(
    ...
    tip_length = 0.02,
    y_position = seq(
      from = max(data$Delta_Ct) * 0.99,
      length.out = length(comparisons_list),
      by = max(data$Delta_Ct) * 0.11))
}
